 ...
 **/
var SQLiteClosable = function() {
	var result;
	// Allow the constructor to either invoke the real java constructor, or function as a "wrapping" method that will take
	// a single argument that is a native hyperloop proxy for this class type and just wraps it in our JS type.
	if (arguments.length == 1 && arguments[0].isNativeProxy && arguments[0].isInstanceProxy && arguments[0].isInstanceOf('android.database.sqlite.SQLiteClosable')) {
		result = arguments[0];
	}
	else {
...
SQLiteClosable.prototype = Object.create(SuperClass.prototype);
SQLiteClosable.prototype.constructor = SQLiteClosable;

Object.defineProperty(SQLiteClosable.prototype, 'super', {
	get: function() {
		if (!this._hasPointer) return null;
		return new SQLiteClosable(this.$native.super);
	},
	enumerable: true
});

SQLiteClosable.className = 'android.database.sqlite.SQLiteClosable';
SQLiteClosable.prototype.className = 'android.database.sqlite.SQLiteClosable';

// class property
Object.defineProperty(SQLiteClosable, 'class', {
	get: function() {
		return Hyperloop.getClass('android.database.sqlite.SQLiteClosable');
	},
	enumerable: true,
	configurable: false
});
...
// Allow subclassing
SQLiteClosable.extend = function (overrides) {
	var subclassProxy = Hyperloop.extend('android.database.sqlite.SQLiteClosable');
	...
	SubClass.prototype = Object.create(SQLiteClosable.prototype);
	SubClass.prototype.constructor = SubClass;
	return SubClass;
};

// Cast
SQLiteClosable.cast = function(object) {
	if (!object.$native || !object.$native.isInstanceProxy) return object;
	return new SQLiteClosable(Hyperloop.cast('android.database.sqlite.SQLiteClosable', object.$native));
};
 ...
 **/
SQLiteClosable.prototype.onAllReferencesReleased = function() {
	if (!this._hasPointer) return null;
	...
	if (result.apiName) {
		if (result.apiName === 'android.database.sqlite.SQLiteClosable') {
			return new SQLiteClosable(result);
		} else {
 ...
 **/
SQLiteClosable.prototype.releaseReferenceFromContainer = function() {
	if (!this._hasPointer) return null;
 ...
 **/
SQLiteClosable.prototype.acquireReference = function() {
	if (!this._hasPointer) return null;
 ...
 **/
SQLiteClosable.prototype.close = function() {
	if (!this._hasPointer) return null;
 ...
 **/
SQLiteClosable.prototype.onAllReferencesReleasedFromContainer = function() {
	if (!this._hasPointer) return null;
 ...
 **/
SQLiteClosable.prototype.releaseReference = function() {
	if (!this._hasPointer) return null;
